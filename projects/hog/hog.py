...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if(flag):
        ...
        return sum
    # In this place the hint said that  "If you call it fewer or more than that, it won't be at the right
    # spot in the cycle for the next roll"
    # What I do at very first time didn't call enought time , because I just return 1
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    all_div = False
    while gcd_num <= min:
        if(player_score % gcd_num == 0 and opponent_score % gcd_num == 0):
            all_div = True
        gcd_num += 1
    if all_div:
        return True
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def higiest(score0, score1):
        if who == 1:
            score = score1
        else:
            score = score0
        r_hight = 0
        if running_high < score-last_score:
            print(score-last_score, 'point(s)! The most yet for Player', who)
            r_hight = score-last_score
        else:
            r_hight = running_high
        return announce_highest(who, score, r_hight)
    return higiest
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    ret, max_score = 1, 0
    for i in range(1, 11):
        avearge_dice = make_averaged(roll_dice, trials_count)
